...
class GrabCut:
    ...
    def grab_cut(self, xmin, xmax, ymin, ymax):
        height, width = self.original_image.shape[0], self.original_image.shape[1]
        mask = np.zeros(self.original_image.shape[:2], dtype="uint8")
        print("xmin", xmin)
        print("xmax", xmax)
        print("ymin", ymin)
        print("ymax", ymax)
        rect = (xmin, ymin, ymax+xmin, xmax+ymin)
        print(rect)
    ...
    def plot_outputs(self):
        ...
        plt.imsave(self.output_path + "GrabCut.jpg", output)
...
class Colouring:
    ...
    def thresholding(self):
        ...
        threshold_image = cv2.adaptiveThreshold(blurred, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 25, 1)
...
rnn = RegionDetector(file_name=(file_name + ".jpg"), file_path=file_path, path=output_path)
rnn.load_img()
rnn.load_model()
...
cutter.grab_cut(int(xmin), int(xmax), int(ymin), int(ymax))
cutter.plot_outputs()

file_path = "MultiOutput/" + file_name + "/" + "GrabCut.jpg"
